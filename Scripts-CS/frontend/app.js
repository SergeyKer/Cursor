...
function resolveProcessMenuGroup(meta) {
  if (!meta) return "other";
  const explicit = meta.menu_group ? String(meta.menu_group).trim() : "";
  if (explicit && explicit !== "other") return explicit;
  const fromMap = CODE_MENU_GROUP[normalizeProcessCode(meta.code)];
  if (fromMap) return fromMap;
  if (INCOMING_OPERATOR_CODES.has(normalizeProcessCode(meta.code))) return "incoming_operator";
  const text = `${meta.name || ""} ${meta.code || ""}`.toLowerCase();
  if (/бухгалтер/i.test(text)) return "accounting";
  if (/исходящ|знакомств|пропуск|акци|тариф|сезон|повышен|возобновлен/i.test(text)) {
    return "outgoing_manager";
  }
  if (/входящ/i.test(text) && /оператор/i.test(text)) return "incoming_operator";
  if (/входящ/i.test(text) && /менедж/i.test(text)) return "incoming_manager";
  return explicit || "incoming_manager";
}

function getMenuGroupLabel(meta) {
  const group = resolveProcessMenuGroup(meta);
  if (group === "other") return "";
  const def = MENU_SECTION_DEFS.find((s) => s.id === group);
  if (!def) return "";
  return def.title.replace(/^\d+\.\s*/, "").replace(/:\s*$/, "").trim();
}

function getProcessMenuGroupClass(meta) {
  const group = resolveProcessMenuGroup(meta);
  return group && group !== "other" ? group.replace(/_/g, "-") : "";
}

function createMenuGroupLabelEl(meta) {
  const label = getMenuGroupLabel(meta);
  if (!label) return null;
  const groupClass = getProcessMenuGroupClass(meta);
  const el = document.createElement("span");
  el.className = `process-group-label process-group-label--meta process-group-label--${groupClass}`;
...
function renderProcessListMain(processesMeta, onSelect, processesData) {
  ...
  if (isMobileLayout()) {
    (processesMeta || []).forEach((process) => {
      ...
      card.type = "button";
      const groupClass = getProcessMenuGroupClass(process);
      card.className = groupClass
        ? `process-list-main__card process-list-main__card--${groupClass}`
        : "process-list-main__card";
      const head = document.createElement("span");
      head.className = "process-list-main__card-head";
      ...
      const groupLabel = createMenuGroupLabelEl(process);
      if (groupLabel) head.appendChild(groupLabel);
  ...
  (processesMeta || []).forEach((process) => {
    ...
    nameWrap.className = "process-list-main__name-cell";
    ...
    const groupLabel = createMenuGroupLabelEl(process);
    if (groupLabel) nameWrap.appendChild(groupLabel);
...
function renderProcessList(processesMeta, onSelect) {
  ...
  function render(filter = "") {
    ...
    filtered.forEach((p) => {
      const g = resolveProcessMenuGroup(p);
    ...
    MENU_SECTION_DEFS.forEach((s) => renderSection(s.id, s.title));
...
function renderProcessDetails(processMeta, processesData) {
  ...
  const groupLabelEl = document.getElementById("processGroupLabel");
  if (groupLabelEl) {
    const groupLabel = getMenuGroupLabel(processMeta);
    if (groupLabel) {
      const groupClass = getProcessMenuGroupClass(processMeta);
      groupLabelEl.textContent = groupLabel;
      groupLabelEl.className = `process-header__group-label process-group-label process-group-label--meta process-group-label--${groupClass}`;
    } else {
      groupLabelEl.textContent = "";
      groupLabelEl.className = "process-header__group-label hidden";
    }
  }
