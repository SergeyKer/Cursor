import type { CefrLevelSpec } from '@/lib/cefr/cefrSpecBindings'
import type { Audience, LevelId } from '@/lib/types'

export type GuardMode = 'dialogue' | 'translation' | 'communication'

export type CefrGuardDeps = {
  getCefrSpec: (level: LevelId) => CefrLevelSpec | null
  getCefrDenyWords: (params: { level: LevelId; audience: Audience }) => Set<string>
}
...
function detectEnglishViolations(
  deps: CefrGuardDeps,
  params: {
    text: string
    level: LevelId
  ...
  const spec = deps.getCefrSpec(params.level)
  if (!spec) return []

  const violations: string[] = []
  const deny = deps.getCefrDenyWords({ level: params.level, audience: params.audience })
...
function trimLongEnglishSentences(
  deps: CefrGuardDeps,
  params: { text: string; level: LevelId }
): string {
  const spec = deps.getCefrSpec(params.level)
...
function simplifyDialogueOrTranslationContent(
  deps: CefrGuardDeps,
  params: {
    content: string
    level: LevelId
  ...
  const transformed = lines.map((line) => {
    if (/^\s*Скажи\s*:/i.test(line)) {
      ...
      const simplified = trimLongEnglishSentences(deps, {
    ...
    if (/\?\s*$/.test(line) || /^[A-Za-z].*[.!?]?$/.test(line.trim())) {
      return trimLongEnglishSentences(deps, {
...
export function applyCefrOutputGuardWithDeps(
  deps: CefrGuardDeps,
  params: {
    mode: GuardMode
    content: string
  ...
  const firstViolations = detectEnglishViolations(deps, {
    text: params.content,
    level: params.level,
  ...
  const rewritten =
    params.mode === 'communication'
      ? trimLongEnglishSentences(deps, {
          text: simplifyEnglishText(params.content),
          level: params.level,
        })
      : simplifyDialogueOrTranslationContent(deps, {
          content: params.content,
          level: params.level,
        })

  const secondViolations = detectEnglishViolations(deps, {
    text: rewritten,
    level: params.level,
    audience: params.audience,
  })
