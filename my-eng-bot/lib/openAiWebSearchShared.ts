...
const CURRENT_INFO_PATTERNS = [
  ...
  /что\s+нового/i,
  /кто\s+.*последн(?:ий|яя|ее).*?(тренер|мэр|президент|министр|губернатор)/i,
  /кто\s+.*(текущ(?:ий|ая|ее)|нынешн(?:ий|яя|ее)).*?(тренер|мэр|президент|министр|губернатор)/i,
  ...
  /\brecent\b/i,
  /\bwhat'?s\s+new\b/i,
  /\bwho\s+is\s+the\s+(?:latest|current)\s+(?:coach|manager|mayor|president|minister|governor)\b/i,
