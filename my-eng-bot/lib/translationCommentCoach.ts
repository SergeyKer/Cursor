...
const TRANSLATION_COMMENT_LABEL_RE = /^\s*Комментарий\s*:\s*/i
...
/** Убирает служебный префикс `Комментарий:` (в т.ч. устаревший `Комментарий_ошибка:`). */
export function stripTranslationCommentLabel(text: string): string {
  return text.replace(/^\s*Комментарий(?:_ошибка)?\s*:\s*/i, '')
}
