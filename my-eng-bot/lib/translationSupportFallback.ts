...
export function buildDeterministicTranslationSupportRu(
  ...
  if (mode === 'incomplete') {
    if (surface) {
      if (audience === 'child') {
        return `💡 Хорошее начало: «${surface}» уже использовано верно, осталось дописать перевод целиком.`
      }
      return `💡 Хорошее начало: «${surface}» уже использовано верно, осталось дописать перевод целиком.`
    }
    if (audience === 'child') {
      return '💡 Хороший старт, но перевод пока неполный — добавь остальную часть предложения по образцу.'
    }
    return '💡 Хороший старт, но перевод пока неполный — добавьте остальную часть предложения по образцу.'
  }
  ...
  if (audience === 'child') {
    return `💡 По смыслу ты уже близко к заданию — давай спокойно доведём формулировку (${pointChild}).`
  }
  return `💡 По смыслу вы уже близки к заданию — давайте спокойно доведём формулировку (${pointAdult}).`
}
