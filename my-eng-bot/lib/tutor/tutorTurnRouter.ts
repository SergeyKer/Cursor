...
const CONTINUE_EXACT_RE =
  /^(почему\??|зачем\??|можно\s+пример\??|можно\s+короткий\s+пример\??|напиши\s+ещ[её]\s+примеры|ещё\s+пример|еще\s+пример|как\s+это\s+запомнить\??|что\s+это\s+значит\??|попроще|ещё\s+раз|еще\s+раз|не\s+понял|подробнее|поясни)\s*$/i
