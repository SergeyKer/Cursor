...
export function resolveLessonFooterTopLine(input: LessonFooterTopLineInput): string | null {
  ...
  if (input.comboMilestoneBlocked && combo >= 3) {
    ...
    return input.audience === 'child'
      ...
      ? `COMBO ×${combo}! COMBO растёт — бонус к уровню позже.`
  ...
  if (input.isRepeatWithSavedMedal) {
    ...
    return input.audience === 'child'
      ...
      ? `Счёт идёт! К уровню — только если побьёшь рекорд (${bestTotalXp} XP).`

      : `Счёт идёт. К уровню — только если побьёте рекорд (${bestTotalXp} XP).`
  ...
  return input.audience === 'child'
    ...
    ? 'Верно! Очки урока есть — к уровню без изменений.'
