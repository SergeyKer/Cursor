...
function getMenuGenerationFallbackMessage(reason: LessonRepeatFallbackReason | undefined): string {
  if (reason === 'provider') return '???????? ? ???????? ? ??????. ?????????? ????????????? ???? ??? ???.'
  if (reason === 'parse') return '?????? ??????? ????? ?? ? ??? ???????. ?????????? ????????????? ???? ??? ???.'
  if (reason === 'validation') return '?????? ????????????? ???? ??????? ????????. ????????? ?????????.'
  if (reason === 'no_steps') return '??? ????? ????? ???? ??? ????? ??? ?????????.'
  return '?? ??????? ????????????? ????? ????. ?????????? ??? ???.'
}
...
function buildTutorFallbackBlueprint(topic: string): LessonBlueprint {
  ...
  return {
    ...
    actions: [
      { id: 'examples', label: '???????? ???????' },
      { id: 'fill_phrase', label: '???????? ?????' },
      { id: 'repeat_translate', label: '???????? ?? ??????????' },
      { id: 'write_own_sentence', label: '?????? ???? ???????????' },
    ],
    followups: {
      examples:
        '**???????:**\n' +
...
/** ???? ?????????????? ????? ? `/api/lesson-repeat` ? bypassCache; ?????????? ??????: `lessonMenuGenerateClientTimeoutMs` (??????? ?? `resolveLessonRepeatMenuBypassMaxAttempts`, ??. `lib/lessonProviderTimeouts.ts`). */
...
export default function AppShell({ entryBridge = null, onRuntimeReady }: AppShellProps) {
  ...
  /** ????? useLayoutEffect storage ? ????? hydration mismatch (localStorage ?????? ?? ???????). */
  ...
  /** ??? ?? greetingNonce: ??? ?????????? consume ??? remount (React Strict Mode). */
  ...
  /** ??????? ????????? ???????? structured-????? ?? intro/tips/briefing. */
  ...
  /** ??????? ????? ?? intro: ?????????? stagger ??? remount (key ???????? lessonId). */
  ...
  /** ???? `<audio>` ??????? ?????? ????????? WebRTC-????? ? ????? ? ???? ?????? ???????? ?? remote, ???? ???? ???? ??? `listening` (???/VAD). */
  ...
  /** iPhone / iPad / iPod ? iPadOS ? ?????????? UA (Macintosh + Mobile). */
  ...
  const handleEngvoRealtimeMessage = useCallback(
    async (raw: string) => {
      ...
      if (parsed.type === 'error') {
        const errorMessage = parsed.error?.message ?? '?????? Realtime-??????'
  ...
  const startEngvoCall = useCallback(async () => {
    ...
    try {
      ...
      dataChannel.onopen = () => {
        ...
        if (!sent) {
          setEngvoSessionError('?? ??????? ????????? ????????? Realtime-??????.')
          return
        }
        clearEngvoTimeout(engvoSessionAckTimeoutRef)
        engvoSessionAckTimeoutRef.current = window.setTimeout(() => {
          setEngvoSessionError('OpenAI ?? ?????????? Realtime-??????. ????????? ????/VPN.')
        }, ENGVO_SESSION_ACK_TIMEOUT_MS)
      }
      ...
      if (!sessionResponse.ok || !sessionData.sdp) {
        ...
        throw new Error(fallback || '?? ??????? ???????? SDP answer ?? ???????.')
      }
      ...
      setEngvoSessionError(
        normalizeEngvoRealtimeUserMessage(
          error instanceof Error ? error.message : '?? ??????? ?????? ?????? Engvo'
        )
      )
    } finally {
    ...
    async (
      ...
      try {
        for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
          ...
          try {
            // ?????? ?????? ????? ?? ?????? ?????? (??? ????????? ??????): tenses, sentenceType, topic, level ? ?.?.
            ...
            try {
              ...
              throw new Error(res.ok ? '???????? ????? ???????.' : `?????? ${res.status}: ${res.statusText}`)
            }
            ...
            onRetryStatus?.(RETRY_MESSAGES[attempt] ?? RETRY_MESSAGES[0])
            await sleep(150)
            await sleep(lastError.message.startsWith('???????? ?????') ? RETRY_DELAY_RATE_LIMIT_MS : RETRY_DELAY_MS)
            continue
          } catch (e) {
            clearTimeout(timeoutId)
            const err =
            ...
            onRetryStatus?.(RETRY_MESSAGES[attempt] ?? RETRY_MESSAGES[0])
            await sleep(150)
            const delayMs = lastError.message.startsWith('???????? ?????') ? RETRY_DELAY_RATE_LIMIT_MS : RETRY_DELAY_MS
            await sleep(delayMs)
          }
        }
  ...
  const openTutorLesson = useCallback(
    async (request: {
      ...
      /** ??????? structured-???? ?? ???????? ?????? (????? tutor-resolve-topic). */
  ...
  const startPracticeFromLesson = useCallback(
    (config: PracticeBuildConfig) => {
      // ???????? ???? ????? ??????? ???????? ????????? ?????? ??????? ???????? ??? ???????? ?????:
      // ??? ??????????? ?? ???????? restartChatForNewModeFromMenu ? abandonPracticeSession ? ?????? ??????.
      // ?????????? ??????: ??????? ? ???????? ? ?????????? ????????, ?? ???????? ???? ????? ?????? ?????.
      menuOpenSnapshotRef.current = null
      resetStructuredLessonSession()
      firstMessageRequestIdRef.current += 1
  ...
  const handlePostLessonAction = useCallback(
    async (action: PostLessonAction) => {
      ...
      if (action === 'independent_practice' || action === 'myeng_training') {
        ...
        setLessonOverlay({
          title: action === 'independent_practice' ? '????????' : '?????????? ? Engvo',
          lines: [
            finaleCopy.goalLine ?? '???????? ? ?????????? ????????? ? ?? ????????.',
            '?????? ????? ????????.',
          ],
    ...
    async (
      ...
      const requestTranslateOnce = async (): Promise<AttemptResult> => {
        ...
        try {
          ...
          return {
            ok: false,
            error: data.error ?? (res.status === 502 ? '?????? ??????? ?????? ???????.' : '?? ??????? ????????? ???????.'),
  ...
  function getMenuSummary(includeTopic: boolean = true): string {
    if (settings.mode === 'communication') {
      if (settings.level === 'all') {
        return settings.communicationInputExpectedLang === 'en' ? 'Chat ? Engvo' : '??? ? Engvo'
      }
      ...
      const titlePrefix = lang === 'ru' ? '???' : 'Chat'
    ...
    const levelEntry = LEVELS.find((l) => l.id === settings.level)
    const levelShort = levelEntry ? (levelEntry.label.split(' - ')[0]?.trim() ?? levelEntry.label) : settings.level
    const normalizedLevelShort = settings.level === 'all' ? '??? ??????' : levelShort
  ...
  const homeFooterVoice = React.useMemo(() => {
    ...
    const resolvedHomeVoiceLine = homeVoiceLine?.trim() || '? ????? ?????. ??????????'
  ...
  const tipsFooterStaticText =
    lessonExtraTipsStatus === 'quiz-correct' || lessonExtraTipsStatus === 'quiz-error'
      ? `??????? ???? | ${Math.min(tipsQuizAnsweredCount, 2)}/2`
  ...
  const footerDynamicText = isAccentActive
    ...
    : isLessonIntroActive
      ...
      : isLessonBriefingActive
      ? resolveFooterWithStreakLayer('?????????? ??????? ? ????? ? ???????.', null, null)
      : isStructuredLessonActive
      ? resolveFooterWithStreakLayer(
          structuredLessonCompletionFooterText ??
  ...
  const baseFooterStaticText = isAccentActive
    ? accentFooterView?.staticText ?? '????????????'
    : isVocabularyHubActive
    ? vocabularyFooterView?.staticText ?? (vocabularyByLevelActive ? '????? ?? ???????' : '??????????? ?????')
    : isPracticeActive
    ? practiceFooterView?.staticText ?? '????????'
    : isLessonIntroActive
  ...
  const communicationVoiceOptions = communicationVoiceTabs.map((mode) => {
    if (mode === 'ru') {
      return { mode, label: 'Ru', title: '??????? ????', ariaLabel: '????????? ??????? ????' }
    }
    if (mode === 'en') {
      return { mode, label: 'En', title: 'English input', ariaLabel: '????????? ?????????? ????' }
    }
    return {
      mode,
      label: 'En (mix)',
            ...
            {!isStructuredLessonActive &&
              ...
              <div className="shrink-0 border-b border-[var(--border)] px-3 py-2">
                <div className="w-full rounded-xl border border-[var(--border)] bg-[var(--bg-card)] px-4 py-3 text-center text-sm text-[var(--text)] shadow-sm">
                  ????????? ????????. ????????? ????????? ?????: <strong>{getMenuSummary(true)}</strong>.
                </div>
              </div>
            )}
            {/* ?? iOS ????? ???????? ?????????? ?????? ???????? ????????? ??????????? ?????.
              ...
              <div className="flex min-h-0 flex-1 flex-col">
                ...
                <div className="flex h-full min-h-0 items-center justify-center bg-[linear-gradient(180deg,var(--chat-wallpaper)_0%,var(--chat-wallpaper-soft)_100%)] px-4">
                  <div className="lesson-enter glass-surface w-full max-w-[24rem] rounded-[1.5rem] border border-[var(--chat-section-neutral-border)] bg-white/95 px-4 py-5 text-center shadow-sm">
                    <p className="text-[15px] font-semibold text-[var(--text)]">Engvo ?????????? ????...</p>
                    <p className="mt-2 text-[14px] leading-relaxed text-[var(--text-muted)]">
                      ????: {pendingTutorLessonTitle}. ?????? ?????????? ???????? ??????? ? ??????? ?? ?????????? ??????.
                    </p>
                  </div>
                </div>
                ...
                <Chat
                  ...
                  engvo={{
                    ...
                    assistantIndicatorText: engvoBootstrapServiceIndicatorText ?? 'Engvo ????????...',
                    onStartCall: () => {
                      void startEngvoCall()
                    },
