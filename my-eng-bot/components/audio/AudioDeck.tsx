...
import { useVocabGrokTts } from '@/hooks/useVocabGrokTts'
...
const AUDIO_DECK_METER_BAR_COUNT = 5
...
type AudioDeckProps = {
  ...
  className?: string
  /**
   * `vocab` — Grok/system via vocab prefs (ThinSession only).
   * `browser` — speechSynthesis (practice and default). Stable per mount; do not toggle from pref.
   */
  ttsMode?: 'browser' | 'vocab'
}
...
const AudioDeck = forwardRef<AudioDeckHandle, AudioDeckProps>(function AudioDeck(
  {
    text,
    voiceId,
    playbackKey,
  ...
  // Both hooks always run (Rules of Hooks). ttsMode is stable per mount.
  const vocabTts = useVocabGrokTts(ttsOptions)
  const browserTts = usePracticeTts(ttsOptions)
  const { isPlaying, speedPreset, togglePlay, stop, cycleSpeed } =
    ttsMode === 'vocab' ? vocabTts : browserTts
    ...
    >
      ...
      <div className="flex min-w-0 justify-center">
        <EngvoVoiceMeter
          ...
          barCount={AUDIO_DECK_METER_BAR_COUNT}
