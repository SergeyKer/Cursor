...
/** Только триггер «посчитай по =» в режиме Общение. */
function normalizeEqualsTrigger(text: string): boolean {
  const t = text.trim().toLowerCase()
  return t === '=' || t === 'равно'
}

const MATH_INTENT_RE =
  /(?:^|[\s,.;:!?])(?:посчитай|считай|давай\s+решим|решим(?:\s+пример)?|сейчас\s+будет|будет\s+пример|будет\s+задача|помоги\s+решить|посчитаем|математик|пример\s+сейчас)(?:$|[\s,.;:!?])/i

function isMathIntentMessage(text: string): boolean {
  return MATH_INTENT_RE.test(text)
}

function isMathPendingForEquals(messages: ChatMessage[]): boolean {
  const lastUser = messages.filter((m) => m.role === 'user').pop()
  if (!lastUser || normalizeEqualsTrigger(lastUser.content)) return false

  let lastIntentIdx = -1
  for (let i = 0; i < messages.length; i++) {
    if (messages[i].role === 'user' && isMathIntentMessage(messages[i].content)) lastIntentIdx = i
  }
  if (lastIntentIdx === -1) return false
  for (let i = lastIntentIdx + 1; i < messages.length; i++) {
    if (messages[i].role === 'user' && normalizeEqualsTrigger(messages[i].content)) return false
  }
  return true
}

function hasMathIntentBeforeLastUser(messages: ChatMessage[]): boolean {
  const userMsgs = messages.filter((m) => m.role === 'user')
  if (userMsgs.length < 2) return false
  for (let i = 0; i < userMsgs.length - 1; i++) {
    if (isMathIntentMessage(userMsgs[i].content)) return true
  }
  return false
}

function looksLikeArithmeticOnly(text: string): boolean {
  const t = text.trim().replace(/\s+/g, '')
  if (!t || /[a-zA-Zа-яА-ЯёЁ]/.test(text)) return false
  return /^[\d+\-*/().,]+$/.test(t) && /\d/.test(t) && /[+\-*/()]/.test(t)
}
...
function tokenizeArithmetic(s: string): ArithTok[] | null {
  const out: ArithTok[] = []
  let i = 0
  while (i < s.length) {
    const c = s[i]
    if ((c >= '0' && c <= '9') || c === '.') {
      ...
      if (!/^\d+\.?\d*$|^\.\d+$/.test(rawNum)) return null
...
function parseExprArith(ts: ArithTok[], idx: { v: number }): number {
  let left = parseTermArith(ts, idx)
  while (idx.v < ts.length && ts[idx.v].type === 'op' && (ts[idx.v].v === '+' || ts[idx.v].v === '-')) {
    const op = ts[idx.v].v
    idx.v++
    const right = parseTermArith(ts, idx)
    left = op === '+' ? left + right : left - right
  }
  return left
}

function parseTermArith(ts: ArithTok[], idx: { v: number }): number {
  let left = parseFactorArith(ts, idx)
  while (idx.v < ts.length && ts[idx.v].type === 'op' && (ts[idx.v].v === '*' || ts[idx.v].v === '/')) {
    const op = ts[idx.v].v
    idx.v++
    const right = parseFactorArith(ts, idx)
...
function parseFactorArith(ts: ArithTok[], idx: { v: number }): number {
  if (idx.v >= ts.length) return Number.NaN
  const cur = ts[idx.v]
  if (cur.type === 'op' && cur.v === '-') {
    idx.v++
    return -parseFactorArith(ts, idx)
  }
  if (cur.type === 'op' && cur.v === '+') {
    idx.v++
    return parseFactorArith(ts, idx)
  ...
  if (cur.type === 'op' && cur.v === '(') {
    idx.v++
    const inner = parseExprArith(ts, idx)
    if (idx.v >= ts.length || ts[idx.v].type !== 'op' || ts[idx.v].v !== ')') return Number.NaN
    idx.v++
    return inner
  }
  if (cur.type === 'num') {
...
function safeEvaluateArithmetic(expr: string): number | null {
  ...
  const idx = { v: 0 }
  const v = parseExprArith(tokens, idx)
  if (idx.v !== tokens.length) return null
  if (!Number.isFinite(v)) return null
  return v
}

function extractArithmeticFromText(raw: string): string | null {
  let t = raw.replace(/,/g, '.')
  t = t.replace(new RegExp(MATH_INTENT_RE.source, 'gi'), ' ')
  ...
  while ((m = re.exec(t)) !== null) {
    ...
    if (chunk.length >= 3 && /\d/.test(chunk) && /[+\-*/()]/.test(chunk)) {
...
function extractExpressionBeforeEquals(messages: ChatMessage[]): string | null {
  const userMsgs = messages.filter((m) => m.role === 'user')
  if (userMsgs.length < 2) return null
  const prior = userMsgs.slice(0, -1).map((m) => m.content).join(' ')
  return extractArithmeticFromText(prior)
}

function appendCommunicationContextWarning(
  base: string,
  recentMessages: ChatMessage[],
  options: { reserveForEquals: boolean; audience: 'child' | 'adult' }
): string {
  const remaining = MAX_MESSAGES_IN_CONTEXT - recentMessages.length
  const eff = options.reserveForEquals ? remaining - 1 : remaining
  let line: string | null = null
  if (eff === 7) {
    line =
      options.audience === 'child'
        ? 'До конца контекста осталось 3 твоих сообщения (учитывая ответы ИИ и знак =).'
        : 'До конца контекста осталось 3 ваших сообщения (учитывая ответы ИИ и знак =).'
  } else if (eff === 5) {
    line =
      options.audience === 'child'
        ? 'До конца контекста осталось 2 твоих сообщения.'
        : 'До конца контекста осталось 2 ваших сообщения.'
  } else if (eff === 3) {
    line =
      options.audience === 'child'
        ? 'До конца контекста осталось 1 твоё сообщение.'
        : 'До конца контекста осталось 1 ваше сообщение.'
  }
  if (!line) return base
  return `${base}\n\n${line}`
}

function handleCommunicationMathFlow(params: {
  recentMessages: ChatMessage[]
  lastUserText: string
  audience: 'child' | 'adult'
}): string | null {
  const { recentMessages, lastUserText, audience } = params
  const trimmed = lastUserText.trim()
  const pending = isMathPendingForEquals(recentMessages)

  if (normalizeEqualsTrigger(trimmed)) {
    if (!hasMathIntentBeforeLastUser(recentMessages)) {
      const msg =
        audience === 'child'
          ? 'Сначала напиши, что хочешь посчитать (например «посчитай»).'
          : 'Сначала введите, что нужно посчитать (например «посчитайте»).'
      return appendCommunicationContextWarning(msg, recentMessages, {
        reserveForEquals: false,
        audience,
      })
    }
    const expr = extractExpressionBeforeEquals(recentMessages)
    if (!expr) {
      const msg =
        audience === 'child'
          ? 'Напиши пример числами, потом отправь =.'
          : 'Введите пример числами, затем отправьте =.'
      return appendCommunicationContextWarning(msg, recentMessages, {
        reserveForEquals: false,
        audience,
      })
    ...
    if (result === null) {
      const msg =
        audience === 'child'
          ? 'Не смог разобрать пример. Используй числа и знаки +, −, *, / и скобки.'
          : 'Не удалось разобрать выражение. Используйте числа и знаки +, −, *, / и скобки.'
      return appendCommunicationContextWarning(msg, recentMessages, {
        reserveForEquals: false,
        audience,
      })
    ...
    const resultStr =
      Number.isInteger(result) ? String(result) : String(Math.round(result * 1e12) / 1e12)
    const out =
      audience === 'child'
        ? `Ответ: ${resultStr} (выражение: ${expr})`
        : `Результат: ${resultStr} (выражение: ${expr})`
    return appendCommunicationContextWarning(out, recentMessages, {
      reserveForEquals: false,
      audience,
    })
  }

  if (isMathIntentMessage(trimmed)) {
    const ack =
      audience === 'child' ? 'Понял, жду =' : 'Хорошо, пришлите =, когда будете готовы.'
    return appendCommunicationContextWarning(ack, recentMessages, {
      reserveForEquals: true,
      audience,
    })
  }

  if (pending && looksLikeArithmeticOnly(trimmed)) {
    const ack = audience === 'child' ? 'Ок, жду =' : 'Хорошо, пришлите =.'
    return appendCommunicationContextWarning(ack, recentMessages, {
      reserveForEquals: true,
      audience,
    })
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (mode === 'communication') {
      const mathEarly = handleCommunicationMathFlow({
        recentMessages,
        lastUserText,
        audience,
      })
      if (mathEarly) return NextResponse.json({ content: mathEarly })
    }
