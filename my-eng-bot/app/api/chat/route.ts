...
export async function POST(req: NextRequest) {
  try {
    ...
    const topicChoicePrefix = isTopicChoiceTurn
      ? 'This turn only (applies for ANY tense): the user is naming or hinting at their topic. Do NOT output Правильно, Комментарий, or Повтори. Always try to infer the topic first: ignore typos and wrong tense (e.g. "I wil plai footbal" → football/sport; "tenis", "vialint" → tennis, violin; "plai footbal" → sport). Reply with exactly ONE question about that topic in the required tense (e.g. "Do you play football often?" or "Are you playing football now?"). Only if the message gives NO hint at all (e.g. "sdf", "sss", random letters) reply with ONE short question asking what they mean ("What do you mean? Could you tell me a bit more?"). Your message must be ONLY one question. Ignore all correction rules below for this turn.\n\n'
      : ''
